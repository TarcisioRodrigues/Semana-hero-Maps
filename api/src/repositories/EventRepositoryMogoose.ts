...
class EventRepositoryMongoose implements EventRepository {
  ...
  async update(event: EventClass, id: string): Promise<any> {
  ...
  async findEventsByName(name: string): Promise<EventClass[]> {
    const findEvent = await EventModel.find({
      title: {
        $regex: name,
        $options: 'i',
